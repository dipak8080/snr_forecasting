...
test.beforeEach(async ({ page }) => {
    ...
    await forecastPage.clickOrderWithStoreTab();
...
test('Forecast Visibility Test', async ({ page }) => {
    ...
    await expect(forecastPage.storeIdInputField).toBeVisible();
...
test.only('Auto-Predict Forecast with Store', async ({ page }) => {
    test.setTimeout(60 * 60 * 1000);
    ...
    for (const item of skuItem.skuIds) {
        ...
        // await forecastPage.storeIdInputField.click();
        await forecastPage.enterStoreId('');
        await forecastPage.enterStoreId(String(storeInfo.storeId));
        await forecastPage.storeIdInputField.press('Enter');
